...
namespace SelaFlights
{
    class PrintEdit
    {
        ...
        public string EditReuslts(string[] input, object[] results, QuerryEnum queryEnum)
        {
            ...
            else if (queryEnum == QuerryEnum.SHORTEST_PATH)
                msg = String.Format(msg, input[0], input[1], (string)results[0], (double)results[1]);
            return msg;
        }
    }
    ...
    public class ResultFormats
    {
        ...
        const string MinDelayJourney = "Of all one stop journeys from {0} to {1}, the one with shortest avg arrival delay:" +
                                        "\r\nthrough {2} - avg arrival delay {3} min";
        ...
        public ResultFormats()
        {
            FormatList = new List<KeyValuePair<QuerryEnum, string>>()
            {
                ...
                new KeyValuePair<QuerryEnum, string>(QuerryEnum.SHORTEST_PATH, MinDelayJourney)
            };
